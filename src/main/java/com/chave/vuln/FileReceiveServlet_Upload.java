...
public class FileReceiveServlet_Upload extends VulnBase {
    ...
    public FileReceiveServlet_Upload() {
    }

    public FileReceiveServlet_Upload(TextArea log, TextArea uploadLog, TextArea execLog) {
    ...
    private void poc(String url) {
        try {
            ...
            if (responseCode1 == HttpURLConnection.HTTP_OK) {
                ...
                try {
                    ...
                    if (responseCode2 == HttpURLConnection.HTTP_OK && responseText2.contains(pocFlag)) {
                        logMessage("[+] FileReceiveServlet 文件上传漏洞存在! 成功上传测试文件: " + fileUrl);
                        return;
                    } else {
                        logMessage("[-] FileReceiveServlet 文件上传漏洞不存在, 请尝试手动利用.");
                        return;
                    }
            ...
            logMessage("[-] FileReceiveServlet 文件上传漏洞不存在, 请尝试手动探测. " + e);
        }
    }
            ...
            if (responseCode1 == HttpURLConnection.HTTP_OK) {
                ...
                try {
                    ...
                    if (responseCode2 == HttpURLConnection.HTTP_OK && responseText2.contains(expFlag)) {
                        ...
                        logMessage("[-] FileReceiveServlet 文件上传失败, 请尝试手动利用.");
                        return;
                    }

                } catch (Exception e) {
                    logMessage("[-] FileReceiveServlet 文件上传失败, 请尝试手动利用. " + e);
                    return;
                }
